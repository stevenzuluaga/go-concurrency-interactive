...
export default function VisualizationPage() {
  ...
  const simulateUserRequest = async (userId: number, delayPerUser: number) => {
    return new Promise<number>((resolve) => {
      ...
      setTimeout(() => {
        setUsers((prev) =>
          prev.map((u) => (u.id === userId ? { ...u, status: "requesting" } : u))
        )
      }, 0)
      ...
      setTimeout(() => {
        setUsers((prev) =>
          prev.map((u) => (u.id === userId ? { ...u, status: "processing" } : u))
        )
      }, delayPerUser + 200)
  ...
  const runConcurrent = async () => {
    ...
    const promises = newUsers.map((user, index) =>
      simulateUserRequest(user.id, index * 30) // Pequeño delay para visualización
    )
  ...
  return (
    <div className="flex flex-col gap-8 p-8">
      <div className="space-y-2">
        <h1 className="text-4xl font-bold text-foreground">Simulación: Usuarios Haciendo Solicitudes</h1>
        <p className="text-lg text-muted-foreground">
          Compara cómo se procesan solicitudes simultáneamente vs secuencialmente
        </p>
      </div>
      ...
      <div className="space-y-4 rounded-lg border border-border bg-card p-6">
        <div className="space-y-4">
          <div>
            <label className="text-sm font-semibold mb-2 block">Cantidad de usuarios: {userCount}</label>
            <input
              type="range"
              min="5"
              max="50"
          ...
          <div className="space-y-4 pt-6 border-t border-border">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold">
                {mode === "concurrent" ? "Ejecución Concurrente" : "Ejecución Secuencial"}
              </h3>
              <div className="text-right">
                <p className="text-sm text-muted-foreground">Tiempo total</p>
                <p className="text-2xl font-bold">
                  {totalTime > 0 ? `${totalTime}ms` : isRunning ? "En progreso..." : ""}
                </p>
              </div>
            </div>
            ...
            {totalTime > 0 && (
              <div className="grid gap-4 md:grid-cols-3 pt-4 border-t border-border">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-2xl font-bold">{Math.round(totalTime / userCount)}ms</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-4 rounded-lg border border-green-600 bg-green-600/10 p-6">
          <h2 className="text-xl font-semibold text-green-700">✅ Concurrencia (Go)</h2>
          <div className="space-y-3 text-sm">
            <div>
              <p className="font-semibold mb-1">¿Cómo funciona?</p>
              <p className="text-muted-foreground">
                Todos los usuarios envían sus solicitudes al mismo tiempo. El servidor procesa múltiples
        ...
        <div className="space-y-4 rounded-lg border border-orange-600 bg-orange-600/10 p-6">
          <h2 className="text-xl font-semibold text-orange-700">⚠️ Secuencial</h2>
          <div className="space-y-3 text-sm">
            <div>
              <p className="font-semibold mb-1">¿Cómo funciona?</p>
              <p className="text-muted-foreground">
                Cada usuario espera a que el anterior termine. Las solicitudes se procesan una por una,
                incluso si el servidor está libre.
              </p>
            </div>
            <div>
              <p className="font-semibold mb-1">Ejemplo con 10 usuarios:</p>
              <p className="text-muted-foreground font-mono">
                Tiempo total: ~8000ms<br/>
                (10 usuarios × 800ms)
              </p>
            </div>
            <div>
              ...
              <p className="text-muted-foreground">
                Muy lento. El servidor desperdicia tiempo esperando sin hacer nada.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4 rounded-lg border border-border bg-card p-6">
        <h2 className="text-xl font-semibold">¿Por qué Go es mejor para esto?</h2>
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            <h3 className="font-semibold mb-2 text-green-600">Goroutines</h3>
            <p className="text-sm text-muted-foreground">
              Millones de "usuarios virtuales" pueden ejecutarse simultáneamente en una sola máquina.
            </p>
          </div>
          <div>
          ...
          <div>
            <h3 className="font-semibold mb-2 text-green-600">Sintaxis simple</h3>
            <p className="text-sm text-muted-foreground">
              Solo una palabra clave: <code className="bg-muted px-1 rounded">go</code>. Sin callbacks ni promesas complejas.
            </p>
          </div>
        </div>
      </div>
